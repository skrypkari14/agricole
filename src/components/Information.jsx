    ...
    return (
        <div className='containerBank'>
            ...
            <div className='gridContainer overflow-hidden'>
                ...
                <p className='text-[#C1221F] font-semibold text-sm min-h-9 text-center flex items-center w-full'>
                    Uważaj: oszuści podają się za pracowników banku
                </p>
                <div></div>
                <div className={`collaps text-sm ${isOpenWhite ? 'open' : ''}`}>
                    <p className='font-semibold mb-4'>Uważaj na oszustów, którzy przez telefon:</p>
                    <ul className='pt-[40px]'>
                        <li>
                            <strong>podają się za pracowników</strong> banków lub instytucji finansowych
                            <br />
                            (na telefonie может wyświetлиć się номер, который выглядит как один из номеров bankowych)
                        </li>
                        <li>chcą Cię przestraszyć <strong>zmyśloną historią o podejrzanym przelewie lub kredycie</strong></li>
                        <li>mogą zapewniać, że współpracują z policją i zależy im, żeby <strong>działać szybko</strong></li>
                    ...
                    <p className='pt-[30px]'>
                        <strong>Niczego nie instaluj, не podawaj poufnych danych, nie wykonuj żadnych operacji.</strong> Najlepiej jak najszybciej się rozłącz. Jeśli chcesz się upewnić, że z Twoim rachunkiem wszystko w porządku, skontaktuj się с nami.
                    </p>
            ...
            <div className='flex cursor-pointer min-w-[80px] ml-4 self-start translate-y-[10px]' onClick={toggleCollapsible}>
                <p className='text-[#C1221F] font-semibold text-sm hover:underline'>
                    {isOpenWhite ? 'Zwiń' : 'Rozwiń'}
                </p>
                <div className={`${isOpenWhite ? 'rotate-180' : 'rotate-0'}`}>
                    <RedArrowDown/>
                </div>
            </div>
        </div>
            <div className='bg-white'>
                <div className='bg-[#006675] dark:bg-[#333F52] rounded-t-xl px-7 py-2 flex items-center justify-between gap-4'>
                    <div className='gridContainer overflow-hidden'>
                        ...
                        <div></div>
                        <div className={`collaps text-sm ${isOpenRed ? 'open' : ''}`}>
                            <p className='pb-[40px] text-white'>Chcesz zlecić przelew, zmienić limity transakcji, metodę autoryzacji, a może odblokować dostęp do aplikacji? Zobacz, jak łatwo możesz to zrobić w CA24 eBank. Przygotowaliśmy dla Ciebie podpowiedzi w sekcji <a href='#' className='text-[#007e90]'>Pytania i odpowiedzi</a>.</p>
                        </div>
                    </div>
                    <div className='flex cursor-pointer min-w-[80px] ml-4 self-start translate-y-[10px]' onClick={() => {toggleCollapsibleRed()}}>
                        <p className='text-white font-semibold text-sm hover:underline'>
                            {isOpenRed ? 'Zwiń' : 'Rozwiń'}
                        </p>
                        <div className={`${isOpenRed ? 'rotate-180' : ''}`}>
                            <WhiteArrowDown/>
                        </div>
                    </div>
                </div>
